...
def preview_message():
    ...
    if request.method == 'POST':
        ...
        if template:
            # Render user's custom message safely by escaping it
            output = render_template_string("{{ user_input }}", user_input=template)
    ...
    if request.method == 'POST':
        ...
        if data:
            try:
                ...
                # Safe deserialization - only allow simple data structures
                import json
